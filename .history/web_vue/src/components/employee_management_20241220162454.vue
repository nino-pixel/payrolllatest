...
<select v-model="formData.position" required>
  ...
  <div class="form-group">
    ...
    <input type="tel" v-model="formData.contact_number" required>
  </div>
    ...
    methods: {
      ...
      async submitForm() {
        try {
          ...
          if (this.isEditing) {
            const response = await axios.put(`/api/employees/${this.formData.id}`, formData)
          ...
          this.errorMessage = error.response?.data?.message || 'An error occurred while saving'
        } finally {
          this.loading = false
        }
      },
...
.modal {
  ...
  overflow-y: auto; /* Added scroll to modal overlay */
}
select{
    background-color: #f2e6ff;
    color: #9737e0;
...
.modal-content {
  ...
  overflow-y: auto; /* Added scroll to modal content */
