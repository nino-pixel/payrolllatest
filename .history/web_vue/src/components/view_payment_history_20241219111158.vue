  ...
  export default {
    ...
    searchEmployee() {
      ...
      this.employeeSuggestions = this.employees.filter(emp => 
        emp.name.toLowerCase().includes(this.payslipName.toLowerCase())
      );
    ...
    selectEmployee(employee) {
      ...
      const latestPayment = this.payments
        .filter(p => p.employeeId === employee.id && p.status === 'sent')
    ...
    async generatePayslip() {
      try {
        ...
        const payslipData = {
          employeeId: this.selectedPayslipEmployee.id,
          name: this.selectedPayslipEmployee.name,
          department: this.selectedPayslipEmployee.department,
          position: this.selectedPayslipEmployee.position,
          salary: this.latestSalary,
          paymentDate: this.latestPaymentDate,
          // Add other necessary details
        };
        
        // Call API or service to generate payslip
        // await generatePayslipService(payslipData);
