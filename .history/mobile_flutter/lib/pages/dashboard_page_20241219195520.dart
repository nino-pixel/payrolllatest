...
import '../features/leave/widgets/leave_request_dialog.dart';
import '../features/overtime/widgets/overtime_request_dialog.dart';
import '../features/attendance/widgets/attendance_history.dart';
      ...
      children: [
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: const Text(
              '3',  // TODO: Get actual count from service
              style: TextStyle(
                color: Colors.white,
                fontSize: 8,
              ),
  ...
  onTap: () async {
    ...
    if (result == true && context.mounted) {
      // Refresh the page or update leave balance if needed
             ...
             // Inside HomeTab class, update the QuickActionCard for Overtime
_buildQuickActionCard(
  icon: Icons.schedule,
  title: 'Overtime',
  onTap: () async {
    final result = await showDialog<bool>(
      context: context,
      builder: (context) => const OvertimeRequestDialog(),
    );
    ...
    return Card(
      child: ListTile(
        ...
        trailing: IconButton(
          ...
          onPressed: () async {
            try {
              final payslipData = PayrollService.getHistoricalPayslip(period);
              await PayslipPdfService.generatePayslip(
                employeeName: payslipData.employeeName,
                employeeId: payslipData.employeeId,
                period: payslipData.period,
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('$period payslip downloaded successfully!'),
                    backgroundColor: Colors.green,
                  ),
                );
              }
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('Error downloading payslip: $e'),
                    backgroundColor: Colors.red,
                  ),
                );
              }
